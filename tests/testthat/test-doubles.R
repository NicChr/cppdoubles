...
testthat::test_that("Integers", {
  set.seed(1000)
  x <- sample((-10^2):(10^2), size = 10^3, replace = TRUE)
  y <- sample((-10^2):(10^2), size = 10^3, replace = TRUE)
  testthat::expect_equal(double_equal(x, y),
                         x == y)
  testthat::expect_equal(double_gte(x, y),
                         x >= y)
  testthat::expect_equal(double_lte(x, y),
                         x <= y)
  testthat::expect_equal(double_gt(x, y),
                         x > y)
  testthat::expect_equal(double_lt(x, y),
                         x < y)
...
testthat::test_that("even more tests", {
  testthat::expect_true(`%~==%`(10^-8, 2 * 10^-8))
  testthat::expect_true(`%~==%`(2 * 10^-8, 10^-8))
  options(cppdoubles.tolerance = sqrt(.Machine$double.eps)/100)
  testthat::expect_false(`%~==%`(10^-8, 2 * 10^-8))
  options(cppdoubles.tolerance = sqrt(.Machine$double.eps)/100)
  testthat::expect_false(`%~==%`(2 * 10^-8, 10^-8))
  options(cppdoubles.tolerance = sqrt(.Machine$double.eps))
  testthat::expect_true(`%~==%`(1.1 * 100 * 10^200, 110 * 10^200))
  testthat::expect_true(`%~==%`(110 * 10^200, 1.1 * 100 * 10^200))
  testthat::expect_false(`%~<%`(1.1 * 100 * 10^200, 110 * 10^200))
  testthat::expect_false(`%~>%`(1.1 * 100 * 10^200, 110 * 10^200))
  testthat::expect_true(`%~<=%`(1.1 * 100 * 10^200, 110 * 10^200))
  testthat::expect_true(`%~>=%`(1.1 * 100 * 10^200, 110 * 10^200))
  testthat::expect_true(`%~==%`(0, 0))
  testthat::expect_false(`%~==%`(0, sqrt(.Machine$double.eps)))
  testthat::expect_true(`%~==%`(0, sqrt(.Machine$double.eps)^2))
  testthat::expect_identical(
  ...
  x <- c(NaN, NA_real_, 0, 0.01, -0.001, sqrt(2)^2, -sqrt(2)^2, Inf, -Inf, 10^7, -10^7)
  combs <- expand.grid(x1 = x, x2 = x)
  testthat::expect_identical(`%~==%`(combs$x1, combs$x2),
                             c(NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA,
                               NA, NA, NA, NA, NA, NA, NA, NA, NA, TRUE, FALSE, FALSE, FALSE,
                               FALSE, FALSE, FALSE, FALSE, FALSE, NA, NA, FALSE, TRUE, FALSE,
                               FALSE, FALSE, FALSE, FALSE, FALSE, FALSE, NA, NA, FALSE, FALSE,
  ...
  # Abs diff would work here but rel diff doesn't
  testthat::expect_true(`%~==%`(10^9, 10^9 + 0.002))
  testthat::expect_false(`%~==%`(0, -0.00001))
  testthat::expect_false(`%~==%`(0, 0.00001))
  testthat::expect_false(`%~==%`(0, 10^20))
  testthat::expect_false(`%~==%`(0, -10^20))
  testthat::expect_false(`%~==%`(0, -10^20))
  testthat::expect_true(`%~==%`(10^-9, 2 * 10^-9)) # Default tolerance isnt low enough
  options(cppdoubles.tolerance = sqrt(.Machine$double.eps)/10^4)
  testthat::expect_false(`%~==%`(10^-9, 2 * 10^-9))
  options(cppdoubles.tolerance = sqrt(.Machine$double.eps))
})
